...
bool GameScene::OnCreate()
{
	...
	CoreEngine::GetInstance()->GetCamera()->SetPosition(vec3(0.0f, 0.0f, 3.0f));
	CoreEngine::GetInstance()->GetCamera()->AddLight(new LightSource(vec3(0.0f, 0.0f, 0.2f), vec3(1.0f, 1.0f, 1.0f), 0.1f, 0.5f, 0.5f));
	...
	cubeModel = new Model(ShaderHandler::GetInstance()->GetShader("basicShader"));
	cubeModel->AddMesh(new Mesh(vertexList, TextureHandler::GetInstance()->GetTexture("CheckerboardTexture"), ShaderHandler::GetInstance()->GetShader("basicShader")));
...
{
	cubeModel->SetAngle(cubeModel->GetAngle() + 0.01f);
}
